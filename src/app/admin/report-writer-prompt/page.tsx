...
export default async function ReportWriterPromptPage() {
  const promptPath = path.join(
    process.cwd(),
    'docs',
    'REPORT_WRITER_AGENT_PROMPT.md',
  );
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <div className="mt-2">
          <SectionHeader
            title="Report Writer Agent Prompt"
            description="Internal prompt for drafting a human-reviewed Growth Readiness Report from an agent packet."
          />
        </div>
      </div>
      ...
      <Card className="flex flex-col gap-4">
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-[7px] border border-dashed border-border bg-[#faf9f5] px-4 py-3 text-sm text-muted">
          <p>
            This page reads from <code>docs/REPORT_WRITER_AGENT_PROMPT.md</code>,
            so operators can copy the prompt without opening GitHub.
          </p>
